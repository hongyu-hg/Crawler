...
public class TestHtmlParser extends TestCase {
	...
	private static final Log logger = LogFactory.getLog(TestHtmlParser.class);
	...
	public void testImageVisitor() {
		try {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				logger.debug("testImageVisitor() ImageURL = " + imgLink.getImageURL());
				logger.debug("testImageVisitor() ImageLocation = " + imgLink.extractImageLocn());
				logger.debug("testImageVisitor() SRC = " + imgLink.getAttribute("SRC"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void testNodeFilter() {
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				logger.debug("testNodeFilter() " + list.elementAt(i).toHtml());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void testLinkTag() {
		try {
			...
			Parser parser = new Parser();
			parser.setURL("http://www.newegg.com.cn/SubCategory/970.htm");
			parser.setEncoding(parser.getEncoding());
			NodeList list = parser.extractAllNodesThatMatch(filter);
			Pattern p = Pattern.compile("-([0-9]+)\\.htm#itemGrid1");
			int maxPageNumber = 0;
			for (int i = 0; i < list.size(); i++) {
				LinkTag node = (LinkTag) list.elementAt(i);
				if (node.getLink().endsWith("#itemGrid1"))
					logger.info("testLinkTag() Line is :" + node.getLinkText() + "\t" + node.getLink());
				Matcher m = p.matcher(node.getLink());
				if (m.find()) {
	...
	public void testLinkCSS() {
		try {
			...
			for (NodeIterator e = parser.elements(); e.hasMoreNodes();) {
				...
				logger.info("testLinkCSS()" + node.getText() + node.getClass());
		...
		try {
			...
			for (int i = 0; i <= nodeList.size(); i++) {
				if (nodeList.elementAt(i) instanceof InputTag) {
					...
					logger.info("OrFilter tag name is :" + tag.getTagName() + " ,tag value is:" + tag.getAttribute("value"));
				}
				if (nodeList.elementAt(i) instanceof SelectTag) {
					...
					for (int j = 0; j < list.size(); j++) {
						...
						logger.info("OrFilter Option" + option.getOptionText());
					}
				...
				if (nodeList.elementAt(i) instanceof TableTag) {
					...
					for (int j = 0; j < rows.length; j++) {
						...
						for (int k = 0; k < td.length; k++) {
							logger.info("<td>" + td[k].toPlainTextString());
						}
		...
		try {
			...
			NodeVisitor visitor = new NodeVisitor() {
				public void visitTag(Tag tag) {
					logger.info("testVisitorAll()  Tag name is :" + tag.getTagName() + " \n Class is :" + tag.getClass());
				}
			...
			NodeVisitor visitor = new NodeVisitor() {
				public void visitTag(Tag tag) {
					if (tag instanceof HeadTag) {
						logger.info("visitTag() HeadTag : Tag name is :" + tag.getTagName() + " \n Class is :" + tag.getClass()
								+ "\n Text is :" + tag.getText());
					} else if (tag instanceof TitleTag) {
						logger.info("visitTag() TitleTag : Tag name is :" + tag.getTagName() + " \n Class is :" + tag.getClass()
								+ "\n Text is :" + tag.getText());

					} else if (tag instanceof LinkTag) {
						logger.info("visitTag() LinkTag : Tag name is :" + tag.getTagName() + " \n Class is :" + tag.getClass()
								+ "\n Text is :" + tag.getText() + " \n getAttribute is :" + tag.getAttribute("href"));
					} else {
						logger.info("visitTag() : Tag name is :" + tag.getTagName() + " \n Class is :" + tag.getClass() + "\n Text is :"
								+ tag.getText());
					}
	...
	public void testLinkBean() {
		...
		for (int i = 0; i < urls.length; i++) {
			URL url = urls[i];
			logger.info("testLinkBean() -url  is :" + url);
		}
